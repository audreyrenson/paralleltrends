...
generate_parameters <- function(Tt,
  ...
  Beta_W1 = matrix( rnorm(n = 2*(Tt+1), mean =mu_Beta_W1, sd = sd_Beta_W1), nrow = Tt + 1, byrow = TRUE)# 2 because terms for A + intercept
  Beta_W2 = matrix( rnorm(n = 2*(Tt+1), mean =mu_Beta_W2, sd = sd_Beta_W2), nrow = Tt + 1, byrow = TRUE)# 2 because terms for A + intercept
  ...
  mean_W1 = runif(Tt + 1, min = 0.1, max = 0.5)
  mean_W2 = 0
